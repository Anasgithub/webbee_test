...
class MenuController extends BaseController
{
    ...
    {
        $getAllMenu = MenuItem::where('parent_id',null)->get();
        $resultData = array();
        $i = 0;
        foreach ($getAllMenu as $menu) {
        
            $resultData[$i]['id'] = $menu->id;
            $resultData[$i]['name'] = $menu->name;
            $resultData[$i]['url'] = $menu->url;
            $resultData[$i]['parent_id'] = $menu->parent_id;
            $resultData[$i]['created_at'] = $menu->created_at;
            $resultData[$i]['updated_at'] = $menu->updated_at;
            $resultData[$i]['children'] = $menu->children;
            $i++;
        }


            return response()->json(['status' => 1, 'Data' => $resultData],200);
